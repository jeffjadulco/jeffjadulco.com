...
First, let's declare all our css variables in our main css file. If you don't know which file is it, it's where you put the tailwind imports.

In my website I tried to stick with five colors: _primary_, _secondary_, and _accent_, for both background and texts. This will differ based on your design, but in my case, I already know what colors I need because I designed my website in Figma beforehand.
...
In order to use the css variables we created, we must extent the tailwind configuration.
...
2. Set the checkbox' `checked` attribute to true when the `theme` is equal to `dark`
